...
WebGPU is a work-in-progress graphics API and future web standard for graphics and compute. The intension of this `webgpu-simplified` package is to simplify the process of building WebGPU apps. It is a side project that has come out of the examples included in my recently published book [**"Advanced GPU Graphics with WebGPU 1.0"**](https://drxudotnet.com). It is not a renderer nor a render engine, but simply  a collection of helper functions and interfaces. Unlike a render engine, this mini librray does not alter the code structure and workflow of the original WebGPU applications. 
